# elbuilder Installation Guide

In this guide, we'll make sure that your system is ready to use `elbuilder` to compile and manage PHP versions. The installation process has three steps:

1.  **Install elbuilder**: Installing the `elbuilder` application itself using `pipx`.
2.  **System Prerequisites**: Installing the necessary libraries and tools to compile PHP.
3.  **Verify Installation**: Running `elbuilder doctor` to verify your setup.
...
If you have Python 3.9+ installed, you can install `elbuilder` directly using `pipx` in 1 step. if not, then please read through the next step
...
```sh
pipx install 'git+https://github.com/kemetic-labs/elbuilder.git'
```
...
After installing elbuilder and the system prerequisites, verify your setup by running:
